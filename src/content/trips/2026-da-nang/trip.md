---
slug: 2026-da-nang
title: 시온이와 다낭
titleShort: 다낭
destination:
  countryKo: 베트남
  cityKo: 다낭
  countryCode: VN
  airport: DAD
  airportName: 다낭 국제공항 (DAD)
  airportLat: 16.0439
  airportLng: 108.1994
startDate: 2026-09-04
endDate: 2026-09-10
timezone: Asia/Ho_Chi_Minh
status: planned
focusPersonId: sion
updatedAt: 2026-08-14
summary: |
  시온이와 부모님, 친가 조부모님, 외할머니와 함께하는 다낭·호이안 여행.
  시내 센터 포인트 2박 후 남호이안 빈펄 4박(둘 다 Agoda 예약 확정).
tags:
  - 베트남
  - 다낭
  - 호이안
  - 가족

participants:
  - personId: sion
    role: child
  - personId: mom
    role: organizer
  - personId: dad
    role: organizer
  - personId: paternal-grandma
    role: guest
  - personId: paternal-grandpa
    role: guest
  - personId: maternal-grandma
    role: guest

lodgings:
  - id: hotel-centre-point-danang
    name: 센터 포인트 다낭 호텔 앤 레지던스
    nameLocal: Centre Point Danang Hotel & Residence
    area: 다낭 시내 · Thạch Thang / 한강 인근
    room: "리버뷰 · 110m² · 퀸 1 + 킹 1 + 싱글 2 (성인 6)"
    address: 02 Nguyễn Thị Minh Khai, Thạch Thang, Da Nang 550000
    mapUrl: https://maps.google.com/?q=Centre+Point+Danang+Hotel+Residence
    coverImage: /images/lodging/centre-point-danang.jpg
    lat: 16.0745
    lng: 108.2205
    checkInDate: 2026-09-04
    checkInTime: "14:00"
    checkOutDate: 2026-09-06
    checkOutTime: "12:00"
    phone: "+842363663939"
    notesPublic: |
      ✅ Agoda 예약 확정 (예약번호 1760****156). 대표 투숙객 PARKSUNGGEUN. 성인 6명.
      요금제: Hotel offer — Free Afternoon Tea. 조식 포함. F&B 10% 할인.
      포함: 프리미엄 Wi-Fi, 인피니티 풀·수영장, 피트니스, 주차, 웰컴 드링크, 애프터눈 티 등.
      후지불: 2026-08-26 자동 결제 예정 (환율에 따라 원화 변동).
      🆓 무료 취소: 다낭 현지시간 2026-08-27 23:59까지. 8/28 00:00부터 환불 불가. 노쇼 시 전액.
      특별요청(금연·트윈): 체크인 시 여건에 따라 반영.
      호텔: +84-236-366-3939 · 02 Nguyen Thi Minh Khai.
      9/4 14:20 DAD 도착 후 체크인(14:00~). 9/6 빈펄로 이동.

  - id: hotel-vinpearl-nam-hoi-an
    name: 빈펄 리조트 & 골프 남호이안
    nameLocal: Vinpearl Resort & Golf Nam Hoi An
    area: 남호이안 · Thanh Niên, Hoi An
    room: "3베드룸 오션뷰 빌라 · 슈퍼킹 2 + 싱글 2 (최대 성인 6)"
    address: Thanh Niên, Hoi An, Vietnam 560000
    mapUrl: https://maps.google.com/?q=Vinpearl+Resort+Golf+Nam+Hoi+An
    coverImage: /images/lodging/vinpearl-nam-hoi-an.jpg
    lat: 15.7775
    lng: 108.3965
    checkInDate: 2026-09-06
    checkInTime: "15:00"
    checkOutDate: 2026-09-10
    checkOutTime: "12:00"
    phone: "+842353676888"
    notesPublic: |
      ✅ Agoda 예약 확정 (예약번호 1760****527). 대표 투숙객 PARKSUNGGEUN. 성인 5 + 아동 1(10세).
      요금제: Summer Vibes — 공항 왕복 + 조식 + VinWonders.
      ✅ 조식: 호텔 확답 — 등록 투숙객 6명 전원 포함 (매일). Agoda 문구 「2인」과 달라도 호텔 기준 6인.
      ✅ VinWonders: 호텔 확답 — 6명 함께 1회(01 time) 입장. 이용 일시는 체크인 시 프런트와 선택.
      포함: 수영장·인피니티 풀, Wi-Fi, 주차, 웰컴 드링크, 생수, 피트니스 등.
      후지불: 2026-09-01 자동 결제 예정 (환율에 따라 원화 변동).
      🆓 무료 취소: 호이안 현지시간 2026-09-02 23:59까지. 9/3 00:00부터 환불 불가. 노쇼 시 전액.
      특별요청(금연·트윈): 체크인 시 여건에 따라 반영. 빌라는 슈퍼킹2+싱글2 구성.
      호텔: +84-235-3676-888 · res.vpnharv@vinpearl.com
      9/10 15:55 귀국 — 오전 체크아웃 후 DAD(다낭 공항) 이동.

transports:
  - id: flight-ke457-out
    type: flight
    label: 가는 편
    operator: 대한항공
    flightNumber: KE457
    cabin: 일반석 세이버 (예약 등급 U)
    from:
      name: 서울/인천 터미널 2
      code: ICN
    to:
      name: 다낭 터미널 2
      code: DAD
    departAt: "2026-09-04T11:40:00+09:00"
    arriveAt: "2026-09-04T14:20:00+07:00"
    notesPublic: 예약 확정. A321-neo · 약 4시간 40분. 기내 전원 공급. PNR·여권 정보는 사이트 미기재.

  - id: flight-ke458-return
    type: flight
    label: 오는 편
    operator: 대한항공
    flightNumber: KE458
    cabin: 일반석 세이버 (예약 등급 T)
    from:
      name: 다낭 터미널 2
      code: DAD
    to:
      name: 서울/인천 터미널 2
      code: ICN
    departAt: "2026-09-10T15:55:00+07:00"
    arriveAt: "2026-09-10T22:30:00+09:00"
    notesPublic: 예약 확정. A321-neo · 약 4시간 35분. 남호이안 → 공항 이동 여유.

links:
  - label: 주베트남 대한민국 대사관 (하노이) · 긴급 안내
    url: https://overseas.mofa.go.kr/vn-ko/index.do
    kind: emergency
  - label: 주호치민 대한민국 총영사관 (중·남부 영사)
    url: https://overseas.mofa.go.kr/vn-hcm-ko/index.do
    kind: emergency
  - label: 호이안 올드타운 (참고)
    url: https://whc.unesco.org/en/list/948/
    kind: reference

packing:
  - item: 여권
    for: all
    essential: true
  - item: 유아용 선크림·모자
    for: [sion]
    essential: true
  - item: 수영복
    for: all
  - item: 편한 신발 (올드타운 산책)
    for: all
    essential: true
  - item: 가벼운 겉옷 (기내·저녁)
    for: all

notesPublic:
  - "대한항공 일정 예약 확정 (KE457 / KE458). 예약번호·여권은 사이트에 올리지 않습니다."
  - "숙소 2구간 — 센터 포인트(시내) 2박 → 남호이안 빈펄 4박(둘 다 Agoda 확정). 이동 1번."
  - "센터 포인트 무료 취소: 다낭 시간 2026-08-27 23:59까지. 결제 예정 2026-08-26."
  - "빈펄 무료 취소 기한: 호이안 시간 2026-09-02 23:59까지. 그 이후 환불 불가."
  - "빈펄 호텔 확답: 조식 6인 포함 · VinWonders 6명 1회(체크인 시 일정 선택)."
  - "시내 2박 — 1안 힐링(동선 최소·마사지·유람선) / 2안 뷰·미식(선짜·씨푸드·야시장·용다리)."
  - "리조트 4박 — 1안 밸런스(빈원더스 주간 + 올드타운 미식) / 2안 호캉스·야간(올드타운 야경 + 빈원더스 오후·쇼). 빈원더스 티켓 1회만 — 사용 일에 동선이 갈림."
  - "어르신 컨디션·시온이 놀이·미식을 하루씩 교차. 한낮 더위·낮잠 우선. 하루 한 코스만."
  - "응급 — 베트남 경찰 113 · 응급의료 115. 호텔 프론트에 먼저 연락."
---

시온이와 함께하는 다낭·호이안 여행입니다.
시내 센터 포인트 2박과 남호이안 빈펄 4박 모두 **1안(밸런스) / 2안(호캉스·야간)** 중 고를 수 있습니다.
빈원더스는 6명 1회이므로, 주간(1안·4일) vs 오후·야간 쇼(2안·5일) 중 언제 쓸지가 핵심입니다.
